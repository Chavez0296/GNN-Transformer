#!/usr/bin/env python3
# test_sage_lympho_cv_prauc.py - GraphSAGE CV with PR-AUC early stopping
...
out_dir = os.path.join(RESULTS_ROOT, "legacy_tests", "sage_cv_prauc")
seed = 42
...
def train_neighbor(
    ...
    for ep in range(1, epochs + 1):
        ...
        if ep % 5 == 0 or ep == 1:
            print(
                f"[SAGE][fold={fold_id}][{ep:03d}/{epochs}] loss={mean_loss:.4f} "
    ...
    _plot_learning_curves(
        fold_dir,
        history,
        f"GraphSAGE CV Fold {fold_id} Learning Curves",
        "sage_learning_curves.png",
        ema=curve_ema,
    )
    ...
    print(
        f"[SAGE][fold={fold_id}] best_epoch={best_epoch} "
        f"val_thr={best_thr:.2f} val_thr_f1={best_thr_f1:.4f} val_thr_acc={best_thr_acc:.4f}"
    )
    print(f"[SAGE][fold={fold_id}] test_acc={acc:.4f} test_f1={f1:.4f}")
    print(
        "[SAGE][fold={}] Test classification report:\n".format(fold_id),
        classification_report(yt_te, yp_te, digits=3, zero_division=0),
    )
    print(f"[SAGE][fold={fold_id}] Test ROC-AUC: {roc_auc:.4f}")
    print(f"[SAGE][fold={fold_id}] Test PR-AUC: {pr_auc:.4f}")

    fpr, tpr, _ = roc_curve(yt_te, y_score)
    precision, recall, _ = precision_recall_curve(yt_te, y_score)

    if roc_auc > 0:
        plot_roc_curve(fold_dir, fpr, tpr, roc_auc, "GraphSAGE ROC Curve", "sage_roc.png")
    ...
    if pr_auc > 0:
        ...
        plt.title("GraphSAGE PR Curve")
        plt.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(fold_dir, "sage_pr.png"), dpi=150)
        plt.close()
...
for fold_id, (train_val_idx, test_idx) in enumerate(skf.split(all_idx, y_all), start=1):
    ...
    if run_eda and fold_id == 1:
        eda(fold_dir, x_all[tr_mask], y_all[tr_mask], x_all, edge_index)
...
summary_path = os.path.join(run_root, f"sage_cv_summary_{run_id}.csv")
...
thresholds_path = os.path.join(run_root, f"sage_cv_thresholds_{run_id}.csv")
...
plt.xlabel("False Positive Rate")
plt.ylabel("True Positive Rate")
plt.title("GraphSAGE CV ROC Curves")
...
plt.xlabel("Recall")
plt.ylabel("Precision")
plt.title("GraphSAGE CV PR Curves")
plt.legend(fontsize=8)

plt.tight_layout()
summary_plot_path = os.path.join(run_root, "sage_cv_roc_pr_summary.png")
